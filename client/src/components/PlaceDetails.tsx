...
// Add type definition for business status
...
export function PlaceDetails({ place, isOpen, onClose }: PlaceDetailsProps) {
  ...
  const addToTripMutation = useMutation({
    mutationFn: async () => {
      if (!place?.place_id || !selectedTripId) {
      ...
      if (!response.ok) {
        ...
        throw new Error(error || 'Failed to add place to trip');
      }
  ...
  const getStatusColor = (status: BusinessStatus | undefined) => {
    switch (status) {
      case 'OPERATIONAL':
                ...
                <>
                  ...
                  <div className="space-y-2">
                    ...
                    {place?.business_status && (
                      <p className="text-sm">
                        Status:{' '}
                        <span className={`capitalize ${getStatusColor(place.business_status)}`}>
          ...
          <div className="py-4">
            {trips.length > 0 ? (
              ...
              <p className="text-muted-foreground text-center">
                No trips found. Create a trip first to add places.
              </p>
            )}
            {selectedTripId && (
              <Button
